...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `event_view` AS SELECT `event`.`event_id` AS `event_id`, `event`.`client_id` AS `client_id`, `event`.`model` AS `model`, `event`.`phone` AS `phone`, `event`.`qq` AS `qq`, `event`.`contact_preference` AS `contact_preference`, `event`.`problem` AS `problem`, `event`.`member_id` AS `member_id`, `event`.`closed_by` AS `closed_by`, `event`.`gmt_create` AS `gmt_create`, `event`.`gmt_modified` AS `gmt_modified`, `event_status`.`status` AS `status` FROM (( `event` LEFT JOIN `event_event_status_relation` ON (( `event`.`event_id` = `event_event_status_relation`.`event_id` ))) LEFT JOIN `event_status` ON (( `event_event_status_relation`.`event_status_id` = `event_status`.`event_status_id` )));

-- ----------------------------
-- View structure for member_view
-- ----------------------------
DROP VIEW IF EXISTS `member_view`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `member_view` AS select `member`.`member_id` AS `member_id`,`member`.`alias` AS `alias`,`member`.`password` AS `password`,`member`.`name` AS `name`,`member`.`section` AS `section`,`member`.`profile` AS `profile`,`member`.`phone` AS `phone`,`member`.`qq` AS `qq`,`member`.`avatar` AS `avatar`,`member`.`created_by` AS `created_by`,`member`.`gmt_create` AS `gmt_create`,`member`.`gmt_modified` AS `gmt_modified`,`role`.`role` AS `role`, member.logto_id from ((`member` left join `member_role_relation` on((`member`.`member_id` = `member_role_relation`.`member_id`))) left join `role` on((`member_role_relation`.`role_id` = `role`.`role_id`)));
